...
export default function ForgotPasswordScreen() {
  ...
  const handleForgotPassword = async () => {
    try {
      ...
      if (!phoneNumber) {
        Alert.alert(t('error', { ns: 'common' }), t('enterPhoneNumber', { ns: 'auth' }));
        return;
      }
      ...
      Alert.alert(t('success', { ns: 'common' }), t('otpSent', { ns: 'auth' }), [
        {
          text: t('ok', { ns: 'common' }),
          onPress: () => {
      ...
      const errorMessage =
        error.response?.data?.message || error.message || t('networkError', { ns: 'auth' });
