  ...
  error => {
    ...
    if (__DEV__) {
      if (error.message === 'Network Error') {
        ...
        if (
          (error.response.status === 404 && error.response.data?.message?.includes('not found')) ||
...
export const authAPI = {
  ...
  async updateProfile(userData: {
    ...
    try {
      ...
      // تحويل البيانات لتتوافق مع توقعات الخادم - الخادم يتوقع fullName وليس firstName
      const dataToSend = {
        fullName: userData.firstName, // الخادم يتوقع fullName بدلاً من firstName
  ...
  async uploadProfileImage(
    ...
    try {
      ...
      const response = await api.post('/api/mobile/auth/upload-profile-image', formData, {
        headers: {
...
export const adsAPI = {
      ...
      try {
        ...
        if (innerError.response) {
          ...
          console.error(
            ...
            typeof innerError.response.data === 'string'
  ...
  async getMyAds(params?: {
    ...
    try {
      ...
      if (__DEV__) {
        console.log(
          'Dev Only - Token info for getMyAds:',
  ...
  async updateAd(adId: string, adData: Partial<Ad>): Promise<ApiResponse<Ad>> {
    try {
      ...
      return {
        success: false,
        message:
          serverErrorMessage || `خطأ أثناء تحديث الإعلان (${error.response?.status || 'unknown'})`,
      };
    }
  },
