...
import i18n, { RTL_LANGUAGES, loadSavedLanguage, changeLanguage } from './i18n/index';
...
export default function LanguageSelect() {
  ...
  const handleContinue = async () => {
    try {
      ...
      Alert.alert('خطأ', 'حدث خطأ أثناء حفظ اللغة', [{ text: 'حسناً' }]);
    } finally {
      setLoading(false);
    }
  };
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: appColors.background }]}>
      ...
      <View style={styles.welcomeContainer}>
        <Text
          style={[
            styles.welcomeText,
            { fontSize: getResponsiveFontSize(28), color: appColors.text },
          ]}
        >
          {selectedLanguage === 'ar'
            ? 'مرحباً بك'
      ...
      <View style={styles.promptContainer}>
        <Text
          style={[
            styles.promptText,
            { fontSize: getResponsiveFontSize(17), color: appColors.textSecondary },
          ]}
        >
          {selectedLanguage === 'ar'
            ? 'اختر لغتك المفضلة'
      ...
      <View style={styles.nextButtonContainer}>
        ...
        >
          ...
          <Ionicons name={arrowDirection} size={16} color="#fff" style={styles.nextButtonIcon} />
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
